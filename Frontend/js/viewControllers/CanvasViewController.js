...
class CanvasViewController extends BaseViewController {
  ...
  /**
   * Handle canvas mouse down - start creating or dragging
   */
  handleCanvasMouseDown(e) {
    ...
    this.createStartX = e.clientX - rect.left;
    this.createStartY = e.clientY - rect.top;

    // Create preview
    this.dragPreview = document.createElement('div');
    this.dragPreview.className = 'drag-preview';
    this.dragPreview.style.position = 'absolute';
    this.dragPreview.style.left = this.createStartX + 'px';
    this.dragPreview.style.top = this.createStartY + 'px';
    this.dragPreview.style.width = '0px';
    this.dragPreview.style.height = '0px';
    this.dragPreview.style.border = '2px dashed #4287f5';
    this.dragPreview.style.backgroundColor = 'rgba(66, 135, 245, 0.1)';
    this.dragPreview.style.pointerEvents = 'none';
    this.canvas.appendChild(this.dragPreview);
  }

  /**
   * Handle note mouse down - start dragging
   */
  handleNoteMouseDown(e, noteId) {
    ...
    this.noteStartX = noteRect.left - rect.left;
    this.noteStartY = noteRect.top - rect.top;
  ...
  handleCanvasMouseMove(e) {
    if (this.isCreating && this.dragPreview) {
      const rect = this.canvas.getBoundingClientRect();
      const currentX = e.clientX - rect.left;
      const currentY = e.clientY - rect.top;

      const width = Math.abs(currentX - this.createStartX);
      const height = Math.abs(currentY - this.createStartY);
      const left = Math.min(currentX, this.createStartX);
      const top = Math.min(currentY, this.createStartY);
    ...
    if (this.isDragging && this.selectedNoteId) {
      const deltaX = e.clientX - this.dragStartX;
      const deltaY = e.clientY - this.dragStartY;

      const newX = Math.max(0, this.noteStartX + deltaX);
      const newY = Math.max(0, this.noteStartY + deltaY);

      const view = this.stickyNoteViews.get(this.selectedNoteId);
      if (view) {
        view.updatePosition(newX, newY);
      }
    }
  }
  ...
  handleCanvasMouseUp(e) {
    if (this.isDragging && this.selectedNoteId) {
      const view = this.stickyNoteViews.get(this.selectedNoteId);
      if (view) {
        const noteData = this.service.getStickyNoteById(this.selectedNoteId);
        if (noteData) {
          // Save position
          this.saveNoteToDatabase(this.selectedNoteId, {
            x: noteData.x,
            y: noteData.y
          });
        }
    ...
    if (this.isCreating && this.dragPreview) {
      const minSize = 50;
      const width = parseInt(this.dragPreview.style.width);
      const height = parseInt(this.dragPreview.style.height);

      if (width > minSize && height > minSize) {
        const x = parseInt(this.dragPreview.style.left);
        const y = parseInt(this.dragPreview.style.top);
        this.createStickyNote(x, y, width, height);
      }

      this.canvas.removeChild(this.dragPreview);
      this.dragPreview = null;
      this.isCreating = false;
    }
  }
